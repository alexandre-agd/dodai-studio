...
export const ProjectWizard: React.FC<ProjectWizardProps> = ({ onComplete }) => {
  ...
  const handleComplete = () => {
      onComplete({ ...data, recommendation: result.title, resultCase: result.case as 1 | 2 | 3 | 4 });
  };
                ...
                {step === 'challenge' && (
                    <div className="w-full">
                        ...
                        <div className="flex flex-col gap-3">
                            ...
                            {(data.stage === 'stage1' || data.stage === 'stage2') && t.wizard.options.q3_early.map((opt: { id: string; title: string }) => (
                                <OptionButton 
                                    key={opt.id}
                            ...
                            {data.stage === 'stage3' && t.wizard.options.q3_ready.map((opt: { id: string; title: string }) => (
                                <OptionButton 
                                    key={opt.id}
                                    onClick={() => handleNext('challenge', opt.id, 'result')} 
                                    title={opt.title}
                            ...
                            {data.stage === 'stage4' && t.wizard.options.q3_open.map((opt: { id: string; title: string }) => (
                                <OptionButton 
                                    key={opt.id}
                                    onClick={() => handleNext('challenge', opt.id, 'result')} 
                                    title={opt.title}
