...
import { Check, ArrowRight, X, Info, Sparkles } from 'lucide-react';
import { useLanguage } from '../contexts/LanguageContext';
import { ServiceTierProps, StandaloneServiceProps } from '../types';

export const Services: React.FC = () => {
  const { t } = useLanguage();
  const { diagnostic, build, run, addons, addonsDisclaimer } = t.services;
        ...
        <div className="mb-24">
            ...
            <div className="grid lg:grid-cols-3 gap-6 items-start">
                ...
                <div className="lg:mt-8">
                     <BuildTierCard tier={(build as ServiceTierProps[])[0]} />
                </div>
                
                {/* Complete (Hero) */}
                <div className="relative z-20 transform lg:-translate-y-4">
                     <BuildTierCard tier={(build as ServiceTierProps[])[1]} />
                </div>

                {/* Premium */}
                <div className="lg:mt-8">
                     <BuildTierCard tier={(build as ServiceTierProps[])[2]} />
                </div>
            </div>
        </div>
